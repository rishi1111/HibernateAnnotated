...
public class TestExample {
	...
	public void addItems() {
		...
		Session session = factory.getCurrentSession();
			Transaction tx = session.beginTransaction();
			Item i = new Item();
			i.setName("Nokia 66001");
			i.setInitialPrice(4500);
			i.setDescription("rock");
			session.merge(i);
	...
	public void linkItemToCategory() {
		...
		Session session = factory.getCurrentSession();
		Transaction tx = session.beginTransaction();
		Category c = (Category) session.get(Category.class, 183); //Mobiles category
		Item i = (Item) session.get(Item.class, 0); //Nokia mobile phone
